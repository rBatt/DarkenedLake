...
FigureFolder <- paste("Figures/Figures_", "v0.5.0", sep="")
...
if(Save){
	if(SaveType==".pdf"){pdf(file=paste(paste("zoop_byTaxon_2010&2012_", Version, sep=""), ".pdf", sep=""), height=3.4, width=3.4)}
	if(SaveType==".png"){png(file=paste(paste("zoop_byTaxon_2010&2012_", Version, sep=""), ".png", sep=""), units="in", res=200, height=3.4, width=3.4)}
	if(SaveType==".eps"){setEPS();postscript(file=paste(paste("zoop_byTaxon_2010&2012_", Version, sep=""), ".eps", sep=""), height=3.4, width=3.4)}
}else{
	dev.new(height=3.4, width=3.4)
}
par(mfrow=c(2,2), mar=c(2,2.25,0,0), cex=1, ps=9, family="serif", mgp=c(3,0.3,0), tcl=-0.25)
...
for(i in 1:4){
	...
	legend("topleft", legend=tt[i], bty="n", inset=c(-0.2, -0.1))
}
mtext(bquote(Biomass~(g/m^2)), side=2, line=-1.25, outer=TRUE)
...
for(i in 1:length(GraphLayers)){
	...
	par(family="Times", las=1, mfrow=c(2,1), mar=c(2.1,4,0.1,0.1), oma=c(1.5,0,0,0), cex=PubCex, ps=9)
