...
ExcludeTaxa <- c("POM","Nija","YWP","PKS","BHD2","Mesocyclops")
ExcludeSamples <- ("O-0362", "V-0270", "P-1202", "P-1166", "O-0382", "P-1165", "P-1206", "P-1238", "P-1239", "P-1243", "Z-1110", "Z-1115", "Z-1195", "Z-1170", "O-0405")
EndMemberTaxa <- c("Periphyton","DOM","Macrophyte","Terrestrial")
InvertTaxa <- c("Zooplankton","Snail")

BaseData <- droplevels(subset(Data, Trophic>=0 & !is.element(SampleID, ExcludeSamples) & !is.element(Taxon, ExcludeTaxa) & is.element(Type, c("Periphyton","DOM","Macrophyte","Terrestrial")), select=c("Year","Trophic","Taxon","d13C","d15N","dD")))
...
for(i in 1:length(GraphLayers)){
	...
	if(GraphLayers[i]=="EndMembers"){
		MetaPhytos <- data.frame("Trophic"=0, subset(EndMembers, Taxon=="MetaPhyto"))
		EpiPhytos <- data.frame("Trophic"=0, subset(EndMembers, Taxon=="EpiPhyto"))
	...
	pdf(file=paste(FigureFolder,"/Ward2010&2012_IsoBiPlot_", GraphLayer[i], "_", ThisVersion, ".pdf", sep=""), width=3.5, height=5.25, pointsize=9)
	...
	text(muDataGroup[,IsoNames[nPlots[1:2,1]]], labels=as.character(TaxID), cex=PubCex*CexScale)
	...
	plot(muDataGroup[,IsoNames[nPlots[1:2,2]]], pch="", cex=1.2, xlab="", ylab="", xlim=Lims1[[1]], ylim=Lims1[[2]], bty="l", xaxt=ifelse(2==1, "n", "s"), cex.axis=PubCex)#
	...
	points(muDataGroup[,IsoNames[nPlots[1:2,2]]], pch=ifelse(muDataGroup[,"Trophic"]==0, 21, 22), bg=ifelse(muDataGroup[,"Year"]==2010, "white", "gray85"), cex=2.7*CexScale, xlab=LabelIso[nPlots[1,2]], ylab=LabelIso[nPlots[2,2]], xlim=Lims1[[1]], ylim=Lims1[[2]])
	# text(muDataGroup[,IsoNames[nPlots[1:2,2]]], labels=as.character(1:nrow(muDataGroup)), cex=PubCex)
	text(muDataGroup[,IsoNames[nPlots[1:2,2]]], labels=as.character(TaxID), cex=PubCex*CexScale)
	#}
