...
for(YearMix in c(2010, 2012)){
	...
	if(YearMix==2010){
		Cons <- c("Calanoid", "Chaoborus", "Helisoma trivolvis", "FHM", "DAC", "CMM", "BHD1") #, "PKS", "FHM", "YWP", "CMM", "BHD", "Mesocyclops", "DAC")
		TL <- c(1, 2, 1, 2, 2, 2, 2, 3) #, 2, 2, 3, 2.5, 2.5, 1.5, 1)
		GraphTitle <- c("Skistodiaptomus oregonensis", "Chaoborus spp.", "Helisoma trivolvis", "Pimephales promelas", "Phoxinus spp.", "Umbra limi", "Small Ameiurus melas") #, "Lepomis gibbosus", "Pimephales promelas", "Perca flavescens", "Umbra limi", "Ameiurus melas",  "Mesocyclops spp.", "Phoxinus spp.")
	}else{
	...
	# ConsChoices <- list(
	# 				"Calanoid"=list(c("All Terrestrial", "Epi. Phytoplankton", "Meta. Phytoplankton") , c("All Terrestrial", "All Phytoplankton")),
	# 				"Chaoborus"=list(c("All Terrestrial", "Epi. Phytoplankton", "Meta. Phytoplankton") , c("All Terrestrial", "All Phytoplankton")),
	# 				"Helisoma trivolvis"=list(c("All Terrestrial", "All Macrophytes", "Periphyton"), c("All Terrestrial", "Floating Macrophytes", "Submersed Macrophytes", "Periphyton")),
	# 				"FHM"=list(c("All Terrestrial", "Floating Macrophytes", "Periphyton", "All Phytoplankton"), c("All Terrestrial", "All Macrophytes", "All Phytoplankton", "Periphyton")),
	# 				"DAC"=list(c("All Terrestrial", "Floating Macrophytes", "Periphyton", "All Phytoplankton"), c("All Terrestrial", "All Macrophytes", "All Phytoplankton", "Periphyton")),
	# 				"CMM"=list(c("All Terrestrial", "Floating Macrophytes", "Periphyton", "All Phytoplankton"), c("All Terrestrial", "All Macrophytes", "All Phytoplankton", "Periphyton")),
	# 				"BHD1"=list(c("All Terrestrial", "Floating Macrophytes", "Periphyton", "All Phytoplankton"), c("All Terrestrial", "All Macrophytes", "All Phytoplankton", "Periphyton")),
	# 				"BHD2"=list(c("All Terrestrial", "Floating Macrophytes", "Periphyton", "All Phytoplankton"), c("All Terrestrial", "All Macrophytes", "All Phytoplankton", "Periphyton"))
	# 				)
...
GroupChoose <- 1
if(Save){
	pdf(file=paste(paste("ConsSummary_", "Grouping", GroupChoose, "_", Version, sep=""), ".pdf", sep=""), height=7, width=8.5, pointsize=8, family="Times")
}else{
	dev.new(height=7, width=8.5, pointsize=8, family="Times")
}
...
Ylim <- range(dArrBots, dArrTops)
# dev.new(width=3.4, height=4)
pdf(file=paste(paste("dTerr_", Version, sep=""), ".pdf", sep=""), height=4, width=3.4, pointsize=10, family="Times")
par(mar=c(4,3.5,1,1), ps=12, cex=1)
...
text(x=1:7, y=-0.6, labels=ConsNames, srt=45, xpd=TRUE,adj=c(1.1,1.1), cex=1, font=0)
mtext("Change in allochthony", side=2, line=2.5)
dev.off()



# layout.show(layout(matrix(c(1,2,3,4,4,6,5,5,7), ncol=3, byrow=TRUE), widths=c(2/7, 2/7, 2.8/7, 2/7, 2/7, 3/7, 2/7, 2/7, 3/7)))
NeatConsNames <- c("Calanoid"="Calanoid", "Chaoborus"="Chaoborus", "Helisoma trivolvis"="Snail (H. trivolvis)", "FHM"="Fathead", "DAC"="Dace", "BHD1"="YoY Bullhead", "BHD2"="Lg Bullhead", "CMM"= "Mud Minn.", "PKS"="Pumpkinseed", "YWP"="Perch", "Mesocyclops"="Mesocyclops")
pdf(file=paste(paste("NeatSummary_", Version, sep=""), ".pdf", sep=""), height=7, width=6.811, pointsize=10, family="Times")
layout(matrix(c(1,2,3,4,4,6,5,5,7), ncol=3, byrow=TRUE), widths=c(2/7, 2/7, 2.8/7, 2/7, 2/7, 3/7, 2/7, 2/7, 3/7))
# par(mar=c(2.5,0.5,1,0.5), oma=c(0,2,0,0)), cex=1)
for(i in 1:length(Cons)){
	Yaxt <- ifelse(is.element(i, c(1,4,5)), "s", "n")
	LegPos <- ifelse(is.element(i, c(1,2,5)), "topleft", "topright")
	if(Yaxt=="s"){
		par(mar=c(2.5,2,1,0.5),cex=1)
	}else{
...
# save(list=c("ResourceUse"), file=paste("ResourceUse2010&2012_", Version, ".RData",sep=""))
