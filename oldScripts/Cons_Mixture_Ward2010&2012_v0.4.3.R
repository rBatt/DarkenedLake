...
library("plyr")
UseDiffs <- ddply(.data=ResourceUse[,-2], .variables=c("Consumer", "Grouping"), .fun=quantDiff)



ChosenLowQuant <- aggregate(ResourceUse[,make.names(names(SourceOpts))], by=list("Year"=ResourceUse[,1], "Consumer"=ResourceUse[,3], "Grouping"=ResourceUse[,4]), FUN=quantile, probs=0.05, na.rm=TRUE)
...
save.image("Cons_Mixture_Ward2010&2012_v0.4.3.RData")
