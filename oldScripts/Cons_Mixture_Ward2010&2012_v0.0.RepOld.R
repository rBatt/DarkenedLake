...
Data <- subset(DataRaw, Taxon!="Nija" & !is.element(SampleID, c("O-0362", "V-0270", "P-1202", "P-1166", "O-0382", "P-1165", "P-1206", "P-1238", "P-1239", "P-1243", "Z-1110", "Z-1115", "Z-1195", "Z-1170", "O-0405")) & is.na(FishID) & AnalyzedForBatt2012=="Yes")     # SampleID!="O-0362" & SampleID!="V-0270" & SampleID!="P-1202" & SampleID!="P-1166")

colMeans(subset(Data, is.element(Taxon, c("Calanoid", "POM", "DOM", "Chaoborus")) & Year==2010 & AnalyzedForBatt2012=="Yes", select=c("d15N", "d13C", "dD")))

byYear <- subset(Data, is.element(Taxon, c("Calanoid", "POM", "DOM", "Water", "Chaoborus")), select=c("Year"))[,]
byTax <- subset(Data, is.element(Taxon, c("Calanoid", "POM", "DOM", "Water", "Chaoborus")), select=c("Taxon"))[,]
byAnalyz <- subset(Data, is.element(Taxon, c("Calanoid", "POM", "DOM", "Water", "Chaoborus")), select=c("AnalyzedForBatt2012"))[,]
aggregate(subset(Data, is.element(Taxon, c("Calanoid", "POM", "DOM", "Water", "Chaoborus")), select=c("d15N", "d13C", "dD")), by=list("Previously Analyzed"=byAnalyz, "Year"=byYear, "Taxon"=byTax), FUN=mean)
...
if(YearMix==2010){
	TSources <- c("Alder", "Sedge", "DOM")#, "Tamarack") #c("Alder", "Sedge", "DOM")
}else{
	TSources <- c("Alder", "Sedge")#, "Tamarack")
}
...
#**************************************
#Begin analysis for each set of sources
#**************************************

if(YearMix==2010){
	Cons <- c("Calanoid", "Chaoborus", "Helisoma trivolvis", "PKS", "FHM", "YWP")
	TL <- c(1, 2, 1, 2, 2, 3)
	GraphTitle <- c("Skistodiaptomus oregonensis", "Chaoborus spp.", "Helisoma trivolvis", "Lepomis gibbosus", "Pimephales promelas", "Perca flavescens")
}else{
...
#GraphTitle <- c("Skistodiaptomus Oregonensis", "Chaoborus spp.", "Snail", "Pumpkinseed", "Fathead Minnow", "Yellow Perch")
...
FourthSources <- list(c("Nuphar variegata", "Nymphaea odorata", "Brasenia schreberi", "Periphyton"), c("Nuphar variegata", "Nymphaea odorata", "Brasenia schreberi")) #list(c("Nuphar", "Nymphaea", "Watershield", "Chara", "Potamogeton", "Periphyton"), c("Nuphar", "Nymphaea", "Chara", "Potamogeton", "Watershield"))
...
for(f_Src in 1:2){
	...
	for(i in 1:nSrcs){
		...
		###SHOULD THE LINE BELOW BE THERE? I THINK IT CAUSES AN ERROR WHERE THE SOURCES WITH ONLY 1 END MEMBER HAVE THEIR VARIANCES DUPLICATED FROM THE PREVIOUS MULTI-END-MEMBER-ED SOURCE'S VARIANCE
		###assign(TempName_Var, c(Temp_d13C_Var, Temp_d15N_Var, Temp_dD_Var)) #This line was incorrectly placed in previous versions, moved into if loop (this was copied from _v3, and may have been in _v4)
	}
	...
	#Begin loop iterating through each consumer
