	...
	for(l in 1:length(Day)){i=Day[l]
		...
		if(Comp1==TRUE){ #Is this component available for day i?
			RdDO <- subset(Data,
				 ...
				 V11)[,1] #...give us V11, the ΔO2 due to biology (not diffusion) in µM/minute
				mRdDO[1] <- mean(na.omit(RdDO))
		}#The average respiration/minute for this Component
		#Comp2*
		if(Comp2==TRUE){
			RdDO <- subset(Data,DoY==CompsT[2] & Fract<=(Light[CompsT[2],1]-(1/24)), V11)[,1]
			mRdDO[2] <- mean(na.omit(RdDO))}		
		#Comp3
		if(Comp3==TRUE){
			RdDO <- subset(Data,DoY==CompsT[3] & Fract>=(Light[CompsT[3],2]+(1/24)), V11)[,1]
			mRdDO[3] <- mean(na.omit(RdDO))
		}	
		#Comp4
		if(Comp4==TRUE){
			RdDO <- subset(Data,DoY==CompsT[4] & Fract<=(Light[CompsT[4],1]-(1/24)), V11)[,1]
...
Matrix <- matrix(nrow=length(Start:Stop), ncol=4, data=c(Start:Stop, Resp(Start:Stop), GPP(Start:Stop), NEP(Start:Stop)[,1]), dimnames=list(NULL,c("DoY",'R', 'GPP', 'NEP')))
