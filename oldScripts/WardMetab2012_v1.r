...
for(i in 1:length(unique(Deps))){
	...
	if(i==1){
		Est_BK <- Metabolism(Sonde, zmix=Data01[,"Zmix"], Wind=Data01[,"Wind"])
	}
	if(i!=1){
		Est_BK <- rbind(Est_BK, Metabolism(Sonde, zmix=Data01[,"Zmix"], Wind=Data01[,"Wind"]))
	}	
}
E
...
names(Est_Good_LM) <- c("Year", "DoY", "GPP_LM", "R_LM", "NEP_LM", "sumPAR", "meanTemp", "meanK", "R2_LM")
...
names(Est_Good_LM_2010) <- c("Year", "DoY", "GPP_LM", "R_LM", "NEP_LM", "sumPAR", "meanTemp", "meanK", "R2_LM")
