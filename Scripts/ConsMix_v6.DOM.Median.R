...
if(Plot==TRUE){
	...
	if(DispMu==TRUE){
		if(.Platform$OS.type=="windows"){
			legend("top", legend=paste("mu=", round(bugsOut$median[[1]][1], 2), sep=""), bty="n")
				}else{
					legend("top", legend=paste("µ=", round(bugsOut$median[[1]][1], 2), sep=""), bty="n")
					}	
		}
	...
	if(nSrcs>=2){
		...
		mtext(SrcNames[2], side=3, line=0.5, outer=FALSE, las=0, font=3)
			if(DispMu==TRUE){
			if(.Platform$OS.type=="windows"){
				legend("top", legend=paste("mu=", round(bugsOut$median[[1]][2], 2), sep=""), bty="n")
					}else{
						legend("top", legend=paste("µ=", round(bugsOut$median[[1]][2], 2), sep=""), bty="n")
						}	
			}
		}
	if(nSrcs>=3){
		plot.density(density(bugsOut$sims.matrix[,3], from=0, to=1), main="", xlab="Proportion of Composition", ylab="", bty="l",...)
		mtext("Proportion of Composition", side=1, line=3, outer=FALSE, las=0)
		mtext("Density", side=2, line=3, outer=FALSE, las=0)
		mtext(SrcNames[3], side=3, line=0.5, outer=FALSE, las=0, font=3)
			if(DispMu==TRUE){
			if(.Platform$OS.type=="windows"){
				legend("top", legend=paste("mu=", round(bugsOut$median[[1]][3], 2), sep=""), bty="n")
					}else{
						legend("top", legend=paste("µ=", round(bugsOut$median[[1]][3], 2), sep=""), bty="n")
						}	
			}
	}
	if(nSrcs==4){
		plot.density(density(bugsOut$sims.matrix[,4], from=0, to=1), main="", ylab="", xlab="Proportion of Composition", bty="l",...)
		mtext("Proportion of Composition", side=1, line=3, outer=FALSE, las=0)
		mtext(paste(SrcNames[4], collapse=", "), side=3, line=0.5, outer=FALSE, las=0, font=3)
			if(DispMu==TRUE){
			if(.Platform$OS.type=="windows"){
				legend("top", legend=paste("mu=", round(bugsOut$median[[1]][4], 2), sep=""), bty="n")
					}else{
						legend("top", legend=paste("µ=", round(bugsOut$median[[1]][4], 2), sep=""), bty="n")
						}	
			}
	}
