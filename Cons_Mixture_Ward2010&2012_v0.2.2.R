...
for(YearMix in c(2010, 2012)){
	...
	if(YearMix==2010){
		Cons <- c("Calanoid", "Chaoborus", "Helisoma trivolvis", "FHM", "DAC", "CMM") #, "PKS", "FHM", "YWP", "CMM", "BHD", "Mesocyclops", "DAC")
		TL <- c(1, 2, 1, 2, 2, 2) #, 2, 2, 3, 2.5, 2.5, 1.5, 1)
		GraphTitle <- c("Skistodiaptomus oregonensis", "Chaoborus spp.", "Helisoma trivolvis", "Pimephales promelas", "Phoxinus spp.", "Umbra limi") #, "Lepomis gibbosus", "Pimephales promelas", "Perca flavescens", "Umbra limi", "Ameiurus melas",  "Mesocyclops spp.", "Phoxinus spp.")
	}else{
		Cons <- c("Calanoid", "Chaoborus", "Helisoma trivolvis", "FHM", "DAC", "CMM") #, "PKS", "FHM", "CMM", "BHD", "Mesocyclops", "DAC")
		TL <- c(1, 2, 1, 2, 2, 2) #, 2, 2, 2.5, 2.5, 1.5, 1)
		GraphTitle <- c("Skistodiaptomus oregonensis", "Chaoborus spp.", "Helisoma trivolvis", "Pimephales promelas", "Phoxinus spp.", "Umbra limi") #, "Lepomis gibbosus", "Pimephales promelas", "Umbra limi", "Ameiurus melas",  "Mesocyclops spp.", "Phoxinus spp.")
...
for(i in 1:length(Cons)){
	...
	pdf(file=paste(paste("ConsSummary_", "Grouping", GroupChoose, "_", Version, sep=""), ".pdf", sep=""), height=7, width=7, pointsize=9)
	# dev.new(width=8, height=8)
	par(mfcol=c(3,2), mar=c(2.5,3.5,1,0.5), cex=1)
	ConsChoicesShort <- c("All Terrestrial"="Terr.", "Epi. Phytoplankton"= "Epi. Phyt.", "Meta. Phytoplankton"="Meta. Phyt.", "DOM"="DOM", "All Macrophytes"="Macroph.", "Periphyton"="Periphyton", "Floating Macrophytes"="Float Mac", "Submersed Macrophytes"="Sub Mac", "All Phytoplankton"="Phyto.", "Local Terrestrial"="Local Terr.")
	ConsNameMedium <- c("Calanoid"="Calanoid", "Chaoborus"="Chaoborus", "Helisoma trivolvis"="Helisoma trivolvis", "FHM"="Fathead", "DAC"="Dace", "BHD"="Bullhead", "CMM"= "Mud Minn.", "PKS"="Pumpkinseed", "YWP"="Perch", "Mesocyclops"="Mesocyclops")
for(i in 1:length(Cons)){
	ThisRU0 <- droplevels(subset(ResourceUse, Consumer==Cons[i] & Grouping==GroupChoose & Month=="Pooled"))
	ThisRU00 <- reshape(ThisRU0, varying=list(c("DietF.1.", "DietF.2.", "DietF.3.", "DietF.4.")), times=1:4, ids=1:nrow(ThisRU0), timevar="Source", v.names="Proportion", direction="long")
	...
	boxplot(Proportion~Year+Source, data=ThisRU, col=c("#FA807225","#3A5FCD25"), border=c("red","blue"), at=rep(c(0.5,3,5.5,8),each=2)+c(-.5, .5), show.names=FALSE, outline=FALSE, ylim=c(0,1), lwd=1.5)
	axis(side=1, at=c(0.5,3,5.5,8), labels=ResourceNames)
